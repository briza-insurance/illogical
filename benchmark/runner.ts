...
export function loadConditions(engine: any, casesRoot: string) {
  return readdirSync(casesRoot, { withFileTypes: true })
    .filter((e) => e.isDirectory())
    .map((e) => {
      const dir = join(casesRoot, e.name)
      const expression = JSON.parse(readFileSync(join(dir, 'expression.json'), 'utf-8'))
...
export function contextProducesExpected(condition: Condition, ctx: ContextKey): boolean {
  const expectedTrue = ctx === 'completeTrue' || ctx === 'partialTrue' || ctx === 'fullExecutionTrue'
  try {
...
export async function runBench(
  ...
  tasks: Array<{ name: string; ctx: ContextKey | null; fn: (c: Condition) => () => void }>,
  outputPath: string,
) {
  console.log(`\nRunning: ${name}`)
  ...
  for (const condition of conditions) {
    for (const task of tasks) {
      if (task.ctx !== null && !contextProducesExpected(condition, task.ctx)) continue
