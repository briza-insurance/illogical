...
if (!baselinePath || !currentPath) {
  console.error('Usage: node --import tsx benchmark/compare.ts <baseline.json> <current.json>')
  process.exit(1)
}
...
for (const group of groups) {
  ...
  for (const key of groupKeys) {
    ...
    if (!base) {
      console.log(label.padEnd(46) + '—'.padStart(16) + formatHz(cur.throughput.mean).padStart(16) + 'NEW'.padStart(10))
      continue
    }
    if (!cur) {
      console.log(label.padEnd(46) + formatHz(base.throughput.mean).padStart(16) + '—'.padStart(16) + 'REMOVED'.padStart(10))
      continue
    }
    ...
    let verdict: string
    if (ratio > 1.05)       { verdict = 'faster'; faster++ }
    else if (ratio < 0.95)  { verdict = 'SLOWER'; slower++ }
    else                    { verdict = '~same';  unchanged++ }
