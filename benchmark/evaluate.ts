...
const tasks: Array<{ name: string; ctx: any; fn: (c: Condition) => () => void }> = [
  { name: 'complete-true',        ctx: 'completeTrue',        fn: (c) => () => c.parsed.evaluate(c.completeTrue) },
  { name: 'complete-false',       ctx: 'completeFalse',       fn: (c) => () => c.parsed.evaluate(c.completeFalse) },
  { name: 'partial-true',         ctx: 'partialTrue',         fn: (c) => () => c.parsed.evaluate(c.partialTrue) },
  { name: 'partial-false',        ctx: 'partialFalse',        fn: (c) => () => c.parsed.evaluate(c.partialFalse) },
  { name: 'full-execution-true',  ctx: 'fullExecutionTrue',   fn: (c) => () => c.parsed.evaluate(c.fullExecutionTrue) },
  { name: 'full-execution-false', ctx: 'fullExecutionFalse',  fn: (c) => () => c.parsed.evaluate(c.fullExecutionFalse) },
