...
describe('BatchEvaluator', () => {
  ...
  describe('mergeContext', () => {
    it('properly merges context after evaluation with updated, new, and removed keys', () => {
      ...
      const state = (evaluator as unknown as { state: BatchEvaluatorState })
        .state
      assert.deepEqual(state.lastContext, {
        a: 10,
        b: 'val-b',
        c: true,
      })
  ...
  describe('evaluate', () => {
    ...
    const testCases: EvaluateTestCase[] = [
      ...
      {
        name: 'Duplicate dynamic collection operand (OP_LOAD_LOCAL CSE)',
        options: {
          expressions: {
            exp1: ['==', [1, '$status'], [1, '$status']],
          },
        },
        context: { status: 'active' },
        expectedResults: { exp1: true },
    ...
    for (const {
      ...
      it(name, () => {
        const evaluator = new BatchEvaluator(options)
