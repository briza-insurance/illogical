...
describe('Condition Engine - Expression - Comparison - Not In', () => {
  ...
  describe('evaluate', () => {
    ...
    test('value type', () => {
      let tests = [
        // Truthy
        { left: new Value(0), right: new Collection([new Value(1), new Value(2)]), expected: true },
        { left: new Value('0'), right: new Collection([new Value('1'), new Value('2')]), expected: true },
        // Truthy - Bi-directional
        { left: new Collection([new Value(1), new Value(2)]), right: new Value(0), expected: true },
        { left: new Collection([new Value('1'), new Value('2')]), right: new Value('0'), expected: true },
        // Truthy - non-comparable types
        { left: new Value('0'), right: new Collection([new Value(1), new Value(2)]), expected: true },
        { left: new Value(0), right: new Collection([new Value('1'), new Value('2')]), expected: true },
        // Falsy - explicit
        { left: new Value(1), right: new Collection([new Value(1), new Value(2)]), expected: false },
        { left: new Value('1'), right: new Collection([new Value('1'), new Value('2')]), expected: false },
      ]
    ...
    test('reference/value type', () => {
      let tests = [
        ...
        { left: new Reference('RefA'), right: new Collection([new Value('1')]), expected: true },
        { left: new Reference('RefB'), right: new Collection([new Value(1)]), expected: true },
        { left: new Reference('RefC'), right: new Collection([new Value(1)]), expected: true },
        // Falsy - explicit
  ...
  test('toString', () => {
    let tests = [
      { left: new Value(0), right: new Collection([new Value(1), new Value(2)]), expected: '(0 not in [1, 2])' },
      { left: new Collection([new Value(1), new Value(2)]), right: new Value(0), expected: '(0 not in [1, 2])' },
    ]
