...
describe('Condition Engine - Expression - Comparison - Greater Than', () => {
  describe('constructor', () => {
    // @ts-ignore
    expect(() => new GreaterThan())
      .toThrowError()
    // @ts-ignore
    expect(() => new GreaterThan(5))
      .toThrowError()
    // @ts-ignore
    expect(() => new GreaterThan(5, 5, 5))
      .toThrowError()
  })
  describe('evaluate', () => {
    ...
    test('value type', () => {
      let tests = [
        ...
        // Falsy - explicit
        { left: new Value(0), right: new Value(0), expected: false },
        ...
        { left: new Value(1), right: new Collection([new Value(0)]), expected: false },
        { left: new Value(1), right: new Collection([new Value('0')]), expected: false },
      ]

      for (const test of tests) {
        // @ts-ignore
        expect(new GreaterThan(test.left, test.right).evaluate({}))
          .toBe(test.expected)
      }
    })
