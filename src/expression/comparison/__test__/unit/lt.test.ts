...
describe('Condition Engine - Expression - Comparison - Less Than', () => {
  describe('constructor', () => {
    // @ts-ignore
    expect(() => new LessThan())
      .toThrowError()
    // @ts-ignore
    expect(() => new LessThan(5))
      .toThrowError()
    // @ts-ignore
    expect(() => new LessThan(5, 5, 5))
      .toThrowError()
  })
  describe('evaluate', () => {
    ...
    test('value type', () => {
      ...
      for (const test of tests) {
        // @ts-ignore
        expect(new LessThan(test.left, test.right).evaluate({}))
          .toBe(test.expected)
      }
    })
