...
// WeakMap Set cache for OP_OVERLAP_CONST — keyed on the const array identity.
// When the CompiledExpression (and its consts entries) is GC'd, cached Sets are released automatically.
const overlapSetCache = new WeakMap<Input[], Set<Result>>()
...
export function interpret(compiled: CompiledExpression, ctx: Context): Result {
  ...
  while (i < len) {
    ...
    switch (op) {
      ...
      case OP_IN_CONST: {
        const constArr = consts[numAt(bytecode[++i])]
        const scalar = stack[stackTop--]
        if (scalar === null || scalar === undefined) {
          stack[++stackTop] = false
          break
      ...
      case OP_OR_AND_IN_CONST_2: {
        // bytecode layout: ref1Idx, ref2Idx, N, (setA0 setB0), ..., (setAN-1 setBN-1)
        const ref1Idx = numAt(bytecode[++i])
        const ref2Idx = numAt(bytecode[++i])
        const n = numAt(bytecode[++i])
        const pairsStart = i + 1
        i += n * 2
        ...
        if (
          ...
          for (let j = 0; j < n; j++) {
            const setAIdx = numAt(bytecode[pairsStart + j * 2])
            const setBIdx = numAt(bytecode[pairsStart + j * 2 + 1])
            const setA = consts[setAIdx]
            const setBArr = consts[setBIdx]

            // Check ref1 ∈ setA
            let s1 = overlapSetCache.get(setA)
            if (s1 === undefined) {
              s1 = new Set<Result>(setA)
              overlapSetCache.set(setA, s1)
            }
            if (!s1.has(v1)) {
              continue
            }
            ...
            // Check ref2 ∈ setB
            let s2 = overlapSetCache.get(setBArr)
            if (s2 === undefined) {
              s2 = new Set<Result>(setBArr)
              overlapSetCache.set(setBArr, s2)
            }
            if (s2.has(v2)) {
              found = true
              break
